...
std::vector<std::pair<Point2i, bool>> ProjectionProfile::findMinimumMaximum(DImage* profile) {
	...
	for (int i = 0; i < mat.rows; i++){
		array1.insert(array1.end(), (int)mat.at<float>(i, 0));
	}

	//END CASES
	if (mat.at<float>(0, 0) - mat.at<float>(1, 0) > DELTA)
		array.insert(array.end(), std::make_pair(Point2i(0, (int)mat.at<float>(0, 0)), true));
...
double ProjectionProfile::getPeakValleySum(DImage* profile) {
	...
	if (array.size() == 1 && array[0].second == true){
		return array[0].first.y;
	}

	for (int i = 1; i < (int)array.size() - 1; i++) {
		if (array[i].second == true && array[i-1].second == false && array[i+1].second == false) { // starts by looking at the peaks. 
			if (array[i - 1].first.y < array[i + 1].first.y)
				sum += array[i].first.y - array[i + 1].first.y;
			else
				sum += array[i].first.y - array[i - 1].first.y;
		}
		else if (array[i].second == true && array[i-1].second == false){
			sum += array[i].first.y - array[i - 1].first.y;
		}
		else if (array[i].second == true && array[i + 1].second == false){
			sum += array[i].first.y - array[i + 1].first.y;
	...
	//END CASES
	if (array[(int)array.size() - 1].second == true && array[(int)array.size() - 2].second == false)
		sum += array[(int)array.size() - 1].first.y - array[(int)array.size() - 2].first.y;
	if (array[0].second == true && array[1].second == false)
