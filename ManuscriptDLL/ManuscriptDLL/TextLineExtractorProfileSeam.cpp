...
TextLineExtractorProfileSeam::TextLineExtractorProfileSeam(ProjectionProfile& proj, DImage* prof, DImage* img) {
...
void TextLineExtractorProfileSeam::MedialSeamDrawing(){
	...
	for (int i = 0; i < (int)arr.size(); i++) {
		if (arr[i].second == false) {
			...
			Point2i end(img.cols - 1, arr[i].first.x);
			line(img, start, end, 0, 10);
		}
	}
}	
...
void TextLineExtractorProfileSeam::extract(){
	...
	for (int i = 0; i < l-1; i++) {
			...
			Mat line = _image->getMat().rowRange((int)min_elem, (int)max_elem);			
			...
			DImage img_line(line);
			projector.drawProfile(&img_line,profile,500);
