...
#include "stdext.h"
#include "TextLineExtractorGreySeam.h"
...
/**
* TextLineExtractorGreySeam Constructor
* Input:
*	rows- amount of rows to extract.
*/
TextLineExtractorGreySeam::TextLineExtractorGreySeam(int rows) {
	theRows = rows;
}

/**
* TextLineExtractorGreySeam Destructor
*/
TextLineExtractorGreySeam::~TextLineExtractorGreySeam() {
}
...
* Notes:
*	Must first call setImage with greyscale image.
*/

void TextLineExtractorGreySeam::extract(vector<TextLine*>& text_lines) {
	ComponentExtractorBinary component_extractor;
	vector<ConnectedComponent*> component_list;

	// Binarize the image to calculate the distance map
	DImage* gry = _image->convert(CV_8U);
	...
	// Extract the connected components from the binary image 
...
vector<cv::Point> TextLineExtractorGreySeam::getNextSeam(Mat energy_map) {
	Point min_pt;
...
vector<ConnectedComponent*> TextLineExtractorGreySeam::getSeamComponents(vector<Point> seam, vector<ConnectedComponent*> &component_list) {
...
int TextLineExtractorGreySeam::getMinRow(Mat energy_map, int row, int col, int range) {
...
vector<Point> TextLineExtractorGreySeam::getSeam(Mat energy_map, Point start) {
	vector<Point> seam;
	int row = start.y;
...
Mat TextLineExtractorGreySeam::getEnergyMapFromDistance(Mat dst_map) {
	Mat energy_map = Mat::zeros(dst_map.size(), CV_32FC1);
...
int TextLineExtractorGreySeam::getMinRow(Mat EnergyMap) {
	int minRow = 0;
...
void TextLineExtractorGreySeam::removeTextLineFromEnergeyMap(Mat energy_map, TextLine& text_line) {
	double mx, mn;
...
void TextLineExtractorGreySeam::drawDisplay(vector<Point> seam) {
